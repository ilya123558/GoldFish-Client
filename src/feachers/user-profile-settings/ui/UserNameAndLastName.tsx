...
export const UserNameAndLastName: React.FC<IProps> = ({ lastNameValue, nameValue, onClickHandle }) => {
  return (
    <div className="relative">
      <p className=" text-black text-[22px] font-bold">{lastNameValue} {nameValue}</p>
      <button
        onClick={() => onClickHandle(['name', 'lastName'])}
        className="absolute flex items-center right-0 top-[2px] transition-all border-[1px] border-transparent hover:border-gold text-primary rounded-[9px] p-[3px_5px] active:scale-[0.97]"
      >
